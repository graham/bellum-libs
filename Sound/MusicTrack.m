...
#define kMusicTrackFinishedPlayingNotification @"MusicTrackFinishedPlayingNotification"

@interface MusicTrack (InternalMethods)

static void BufferCallback(void *inUserData, AudioQueueRef inAQ, AudioQueueBufferRef buffer);
- (void)callbackForBuffer:(AudioQueueBufferRef)buffer;
- (UInt32)readPacketsIntoBuffer:(AudioQueueBufferRef)buffer;
- (void)forwardTrackFinishedPlayingNotificationToListener:(id)dummy;
@end
...
- (id)initWithPath:(NSString *)path
{
	...
	if (path == nil) { [self release]; return nil; }
	...
	{
		NSLog(@"MusicTrack Error - initWithPath: could not open audio file. Path given was: %@", path);
                [self release];
		return nil;
	}
	...
	if (dataFormat.mBytesPerPacket == 0 || dataFormat.mFramesPerPacket == 0)
	{
		...
		if (maxPacketSize > gBufferSizeBytes)
		{
			...
			NSLog(@"MusicTrack Warning - initWithPath: had to limit packet size requested for file path: %@", path);
		}
	...
	for (i = 0; i < NUM_QUEUE_BUFFERS; i++)
	{
                ...
                if (stat) {
                    NSLog(@"Error return from AudioQueueAllocateBuffer for %@: %d\n", path, (int)stat);
                    [self release];
                    return nil;
                }
	...
	trackClosed = NO;
        ...
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(forwardTrackFinishedPlayingNotificationToListener:) name:kMusicTrackFinishedPlayingNotification object:self];
...
static void BufferCallback(void *inUserData, AudioQueueRef inAQ, AudioQueueBufferRef buffer)
{
	...
	[(MusicTrack *)inUserData callbackForBuffer:buffer];
}
...
- (void)postTrackFinishedPlayingNotification:(id)object
{
    if (listener)
        [[NSNotificationQueue defaultQueue]
            enqueueNotification:
                [NSNotification 
                    notificationWithName:kMusicTrackFinishedPlayingNotification
                    object:self]
...
- (void) forwardTrackFinishedPlayingNotificationToListener:(id)dummy {
    [[self retain] autorelease]; // just in case listener deletes us, keep us around for autorelease..
    if ([listener respondsToSelector:@selector(musicTrackFinishedPlaying:)])
        [(id<MusicTrackListener>)listener musicTrackFinishedPlaying:self];
}
